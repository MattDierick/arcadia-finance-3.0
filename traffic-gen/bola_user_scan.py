#!/usr/bin/env python3
"""
bola_user_scan.py – Arcadia Finance BOLA traffic generator
===========================================================
Sends exactly 100 GET /api/users/{id} requests — one per BOLA target user
(IDs 5–104, seeded in db/init.sql).
...
lies in the fact that the endpoint returns full profile data (including password)
with zero ownership check — any valid JWT can fetch any ID.

Usage:
    python3 bola_user_scan.py [--url http://localhost] [--delay 0.5]
...
def main():
    parser = argparse.ArgumentParser(
        description="Arcadia Finance – BOLA scan: each of 100 users fetches their own /api/users/{id}",
    ...
    print(f"\n{BOLD}{'═'*60}{RESET}")
    print(f"{BOLD}  Arcadia Finance — BOLA User Scan{RESET}")
    print(f"{'═'*60}")
    print(f"  Target  : {CYAN}{base}{RESET}")
    print(f"  Users   : {len(BOLA_USERS)} (IDs 5–104)")
    print(f"  Endpoint: GET /api/users/{{id}}")
    print(f"  Auth    : unique JWT per user (real secret, passes require_auth)")
    ...
    for user_id, first_name, surname in BOLA_USERS:
        ...
        token    = _make_jwt(user_id, username)
